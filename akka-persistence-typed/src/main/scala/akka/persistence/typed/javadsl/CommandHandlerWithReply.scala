...
final class CommandHandlerWithReplyBuilderByState[Command, Event, S <: State, State] @InternalApi private[persistence]
  ...
  private def addCase
    (
        ...
        handler: BiFunction[S, Command, ReplyEffect[Event, State]])
    : Unit = {
   ...
   */
  def onCommand
    (predicate: Predicate[Command], handler: BiFunction[S, Command, ReplyEffect[Event, State]])
    : CommandHandlerWithReplyBuilderByState[Command, Event, S, State] = {
   ...
   */
  def onCommand
    (predicate: Predicate[Command], handler: JFunction[Command, ReplyEffect[Event, State]])
    : CommandHandlerWithReplyBuilderByState[Command, Event, S, State] = {
    addCase(
  ...
  def onCommand[C <: Command]
    (commandClass: Class[C], handler: BiFunction[S, C, ReplyEffect[Event, State]])
    : CommandHandlerWithReplyBuilderByState[Command, Event, S, State] = {
    addCase(
   ...
   */
  def onCommand[C <: Command]
    (commandClass: Class[C], handler: JFunction[C, ReplyEffect[Event, State]])
    : CommandHandlerWithReplyBuilderByState[Command, Event, S, State] = {
    onCommand[C](
      commandClass,
  ...
  def onCommand[C <: Command]
    (
        ...
        handler: Supplier[ReplyEffect[Event, State]])
    : CommandHandlerWithReplyBuilderByState[Command, Event, S, State] = {
    onCommand[C](
      commandClass,
      new BiFunction[S, C, ReplyEffect[Event, State]] {
  ...
  def onAnyCommand
    (
        handler: JFunction[Command, ReplyEffect[Event, State]])
    : CommandHandlerWithReply[Command, Event, State] = {
    addCase(
      _ => true,
      new BiFunction[S, Command, ReplyEffect[Event, State]] {
   ...
   */
  def orElse[S2 <: State]
    (other: CommandHandlerWithReplyBuilderByState[Command, Event, S2, State])
